...
(* ================== GESTION DE LA MÉMOIRE (MANQUANTE AVANT) ================= *)
...
let main () =
  ...
  for w = 0 to 15 do 
    ...
    Printf.printf "S%d" real_week; flush Stdlib.stdout;
    ...
    let final_solution = 
      ...
      with Properly_sat.Unsatisfiable ->
        ...
        Printf.printf " [Secours]"; s
    in

    (* Mise à jour de l'historique pour la génération suivante *)
  ...
  let csv_ready_list = List.map (fun ra ->
    let nom_p = get_colleur_name ra.r_prof in
    let nom_c = get_time_string ra.r_slot in
    (ra.r_week, ra.r_group, nom_c, nom_p)
  ) optimized_assignments in

  Printf.printf "--- EXPORT CSV ---\n";
  Output.export_to_csv "colloscope_final.csv" csv_ready_list
